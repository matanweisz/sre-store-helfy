...
**AI agent loop** (`ai_service/app.py:investigate`): native function-calling, max 10 iterations, tool results truncated to ~8000 chars (catalog gets 16000). The four tools live in `ai_service/tools.py`: `get_metric_catalog`, `query_prometheus`, `search_logs`, `get_recent_errors`. The system prompt (`prompts.py`) is **domain-agnostic** — it encodes the triage loop and points the agent at `get_metric_catalog` for signal names/baselines; `build_system_prompt()` appends the optional `system_prompt_domain_hint` setting. **Never edit the prompt's loop without updating `guidelines.md` §6** (they must stay aligned). After the loop concludes, `build_report()` runs one extra LLM call to also return a typed `IncidentReport` (`report.py`) on `InvestigateResponse.report` (skip via `structured_output=false`).

**Pluggable LLM client** (`ai_service/llm.py`): the loop talks only to the provider-neutral `LLMClient` seam (`start`/`step`/`add_tool_result`/`structure`, exchanging neutral `AssistantTurn`/`ToolCall`), never a provider's wire format. `LLM_PROVIDER` selects `OpenRouterClient` (default — OpenAI SDK at OpenRouter, model `anthropic/claude-sonnet-4.6`) or `AnthropicClient` (native SDK, default `claude-sonnet-4-6`, adds **prompt caching** on the system+tools prefix via `cache_control` and structured outputs via `output_config.format`). Each client owns its own native message history; add a provider by implementing the protocol + extending `build_client()`. Tests drive the loop with a `FakeLLMClient` (no network) — see `tests/_fakes.py`.

**MCP server** (`ai_service/mcp_server.py`): the same four `tools.py` functions are also exposed over MCP (FastMCP), so any MCP-capable agent can investigate the stack. Console entry point `obs-mcp` (stdio by default; `MCP_TRANSPORT=streamable-http` for networked, bound via `MCP_HOST`/`MCP_PORT`). Tool descriptions are reused from `tools.TOOLS` (single source). Optional compose service `ai-mcp` is gated behind the `mcp` profile (won't start on a plain `docker compose up`). Usage + Claude Desktop/Cursor config: `docs/mcp.md`. **`mcp` pulls `starlette`, which must stay `<0.49` to satisfy FastAPI** — install all deps in one resolution (the Dockerfile/pyproject do) so pip doesn't greedily upgrade starlette and break FastAPI.
